#Variável: Moderate intense distance
...
#reconstrução da variável bruta (mid já vem como taxa DMI/tempo no tratamento)
df$mid_bruto <- df$mid * df$playing_time
...
#modelo 1
mixed_model1 <- lmer(
  mid_bruto ~ position + interval_c + quarter + age_c + playing_time_c +
...
#modelo 2
mixed_model2 <- lmer(
  mid_bruto ~ position + interval_c + quarter + age_c + playing_time_c + playng_venue +
    (1 | player) + (1 | match/qt_match) ,
  data = df, REML = FALSE
)
...
#modelo 3
mixed_model3 <- lmer(
  mid_bruto ~ position + interval_c + quarter + age_c + playing_time_c + playng_venue + opponent_level_c +
    (1 | player) + (1 | match/qt_match) ,
  data = df, REML = FALSE
)
...
#modelo 4
mixed_model4 <- lmer(
  mid_bruto ~ position + interval_c + quarter + age_c + playing_time_c + playng_venue + opponent_level_c + score_dif_c +
    (1 | player) + (1 | match/qt_match) ,
  data = df, REML = FALSE
)
...
tab_model(
  ...
  dv.labels = c("modelo 1 Mid", "modelo 2 Mid", "modelo 3 Mid", "modelo 4 Mid"),
...
grafico_efeitos_fixos <- ggplot(efeitos_fixos, aes(x = estimate, y = reorder(term, estimate), color = sinal)) +
  ...
  labs(x = "Estimativa (Beta)", y = NULL, color = NULL,
       title = "Efeitos fixos - Modelo 4 (Mid)") +
...
grafico_trajetorias <- ggplot() +
  ...
  labs(x = "Quarto", y = "Mid previsto",
       title = "Trajetória individual de Mid por quarto (modelo 4)",
...
grafico_caterpillar <- ggplot(ranef_tidy, aes(x = estimate, y = reorder(level, estimate), color = sinal)) +
  ...
  labs(x = "Desvio em relação à média", y = "Atleta", color = NULL,
       title = "Efeitos aleatórios por atleta - Modelo 4 (Mid)") +
