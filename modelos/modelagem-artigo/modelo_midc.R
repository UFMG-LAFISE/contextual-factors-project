#Variável: Moderate intense deceleration
...
#reconstrução da variável bruta (midc já vem como taxa DCMI/tempo no tratamento)
df$midc_bruto <- df$midc * df$playing_time
...
#modelo 1
mixed_model1 <- lmer(
  midc_bruto ~ position + interval_c + quarter + age_c + playing_time_c +
...
#modelo 2
mixed_model2 <- lmer(
  midc_bruto ~ position + interval_c + quarter + age_c + playing_time_c + playng_venue +
    (1 + quarter_lin | player) + (1 | match/qt_match) ,
  data = df, REML = FALSE
)
...
#modelo 3
mixed_model3 <- lmer(
  midc_bruto ~ position + interval_c + quarter + age_c + playing_time_c + playng_venue + opponent_level_c +
    (1 + quarter_lin | player) + (1 | match/qt_match) ,
  data = df, REML = FALSE
)
...
#modelo 4
mixed_model4 <- lmer(
  midc_bruto ~ position + interval_c + quarter + age_c + playing_time_c + playng_venue + opponent_level_c + score_dif_c +
    (1 + quarter_lin | player) + (1 | match/qt_match) ,
  data = df, REML = FALSE
)
...
tab_model(
  ...
  dv.labels = c("modelo 1 Midc", "modelo 2 Midc", "modelo 3 Midc", "modelo 4 Midc"),
  string.pred = "Preditores",
  string.est = "Estimativa (Beta)",
  title = "Tabela. Modelos Mistos para Midc (Moderate Intense Deceleration) e Fatores Contextuais"
  ,
  file = if (exists("EXPORT_DIR")) file.path(EXPORT_DIR, paste0("tabela_", "midc", ".html")) else NULL
)
...
grafico_efeitos_fixos <- ggplot(efeitos_fixos, aes(x = estimate, y = reorder(term, estimate), color = sinal)) +
  ...
  labs(x = "Estimativa (Beta)", y = NULL, color = NULL,
       title = "Efeitos fixos - Modelo 4 (Midc)") +
  theme_minimal(base_size = 12) +
  theme(legend.position = "top")

print(grafico_efeitos_fixos)
if (exists("EXPORT_DIR")) ggsave(file.path(EXPORT_DIR, "graficos", "midc_efeitos_fixos.png"), grafico_efeitos_fixos, width = 8, height = 6, dpi = 300)
...
grafico_trajetorias <- ggplot() +
  ...
  labs(x = "Quarto", y = "Midc previsto",
       title = "Trajetória individual de Midc por quarto (modelo 4)",
       subtitle = "Linhas cinza: atletas | Linha azul: média da população") +
  theme_minimal(base_size = 12)

print(grafico_trajetorias)
if (exists("EXPORT_DIR")) ggsave(file.path(EXPORT_DIR, "graficos", "midc_trajetorias.png"), grafico_trajetorias, width = 8, height = 6, dpi = 300)
...
grafico_caterpillar <- ggplot(ranef_tidy, aes(x = estimate, y = reorder(level, estimate), color = sinal)) +
  ...
  labs(x = "Desvio em relação à média", y = "Atleta", color = NULL,
       title = "Efeitos aleatórios por atleta - Modelo 4 (Midc)") +
  theme_minimal(base_size = 10) +
  theme(axis.text.y = element_text(size = 6), legend.position = "top")

print(grafico_caterpillar)
if (exists("EXPORT_DIR")) ggsave(file.path(EXPORT_DIR, "graficos", "midc_caterpillar.png"), grafico_caterpillar, width = 8, height = 10, dpi = 300)

#3. DIAGNÓSTICO - pressupostos do modelo 4
diagnostico_modelo4 <- check_model(mixed_model4)
print(diagnostico_modelo4)
if (exists("EXPORT_DIR")) {
  png(file.path(EXPORT_DIR, "pressupostos", "midc_check_model.png"), width = 1600, height = 1400, res = 150)
  print(diagnostico_modelo4)
  dev.off()
}
