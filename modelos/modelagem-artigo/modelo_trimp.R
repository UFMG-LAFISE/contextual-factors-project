...
tab_model(
  ...
  dv.labels = c("modelo 1 TRIMP", "modelo 2 TRIMP", "modelo 3 TRIMP", "modelo 4 TRIMP"),
  ...
  ,
  file = if (exists("EXPORT_DIR")) file.path(EXPORT_DIR, paste0("tabela_", "trimp", ".html")) else NULL
)
...
grafico_efeitos_fixos <- ggplot(efeitos_fixos, aes(x = estimate, y = reorder(term, estimate), color = sinal)) +
  ...
  labs(x = "Estimativa (Beta)", y = NULL, color = NULL,
       title = "Efeitos fixos - Modelo 4 (TRIMP)") +
  theme_minimal(base_size = 12) +
  theme(legend.position = "top")

print(grafico_efeitos_fixos)
if (exists("EXPORT_DIR")) ggsave(file.path(EXPORT_DIR, "graficos", "trimp_efeitos_fixos.png"), grafico_efeitos_fixos, width = 8, height = 6, dpi = 300)
...
grafico_trajetorias <- ggplot() +
  ...
  labs(x = "Quarto", y = "TRIMP previsto",
       title = "Trajetória individual de TRIMP por quarto (modelo 4)",
       subtitle = "Linhas cinza: atletas | Linha azul: média da população") +
  theme_minimal(base_size = 12)

print(grafico_trajetorias)
if (exists("EXPORT_DIR")) ggsave(file.path(EXPORT_DIR, "graficos", "trimp_trajetorias.png"), grafico_trajetorias, width = 8, height = 6, dpi = 300)
...
grafico_caterpillar <- ggplot(ranef_tidy, aes(x = estimate, y = reorder(level, estimate), color = sinal)) +
  ...
  labs(x = "Desvio em relação à média", y = "Atleta", color = NULL,
       title = "Efeitos aleatórios por atleta - Modelo 4 (TRIMP)") +
  theme_minimal(base_size = 10) +
  theme(axis.text.y = element_text(size = 6), legend.position = "top")

print(grafico_caterpillar)
if (exists("EXPORT_DIR")) ggsave(file.path(EXPORT_DIR, "graficos", "trimp_caterpillar.png"), grafico_caterpillar, width = 8, height = 10, dpi = 300)

#3. DIAGNÓSTICO - pressupostos do modelo 4
diagnostico_modelo4 <- check_model(mixed_model4)
print(diagnostico_modelo4)
if (exists("EXPORT_DIR")) {
  png(file.path(EXPORT_DIR, "pressupostos", "trimp_check_model.png"), width = 1600, height = 1400, res = 150)
  print(diagnostico_modelo4)
  dev.off()
}
