#Variável: Moderate intense acceleration (por minuto jogado - miac)
...
#NOTA: aqui a variável dependente é usada como vem da base (miac = ACMI/tempo,
...
#modelo 1
mixed_model1 <- lmer(
  miac ~ position + interval_c + quarter + age_c + playing_time_c +
...
#modelo 2
mixed_model2 <- lmer(
  miac ~ position + interval_c + quarter + age_c + playing_time_c + playng_venue +
    (1 + quarter_lin | player) + (1 | match/qt_match) ,
  data = df, REML = FALSE
)
...
#modelo 3
mixed_model3 <- lmer(
  miac ~ position + interval_c + quarter + age_c + playing_time_c + playng_venue + opponent_level_c +
    (1 + quarter_lin | player) + (1 | match/qt_match) ,
  data = df, REML = FALSE
)
...
#modelo 4
mixed_model4 <- lmer(
  miac ~ position + interval_c + quarter + age_c + playing_time_c + playng_venue + opponent_level_c + score_dif_c +
    (1 + quarter_lin | player) + (1 | match/qt_match) ,
  data = df, REML = FALSE
)
...
tab_model(
  ...
  dv.labels = c("modelo 1 Miac/min", "modelo 2 Miac/min", "modelo 3 Miac/min", "modelo 4 Miac/min"),
  string.pred = "Preditores",
  string.est = "Estimativa (Beta)",
  title = "Tabela. Modelos Mistos para Miac (Moderate Intense Acceleration) por Minuto e Fatores Contextuais"
)
...
grafico_efeitos_fixos <- ggplot(efeitos_fixos, aes(x = estimate, y = reorder(term, estimate), color = sinal)) +
  ...
  labs(x = "Estimativa (Beta)", y = NULL, color = NULL,
       title = "Efeitos fixos - Modelo 4 (Miac/min)") +
...
grafico_trajetorias <- ggplot() +
  ...
  labs(x = "Quarto", y = "Miac/min previsto",
       title = "Trajetória individual de Miac/min por quarto (modelo 4)",
...
grafico_caterpillar <- ggplot(ranef_tidy, aes(x = estimate, y = reorder(level, estimate), color = sinal)) +
  ...
  labs(x = "Desvio em relação à média", y = "Atleta", color = NULL,
       title = "Efeitos aleatórios por atleta - Modelo 4 (Miac/min)") +
