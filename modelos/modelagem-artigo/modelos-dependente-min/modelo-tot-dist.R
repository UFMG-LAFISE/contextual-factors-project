#Variável: Total distance (por minuto jogado - tot_dist)
...
#NOTA: aqui a variável dependente é usada como vem da base (tot_dist = dist_total/tempo,
#já dividida pelo tempo jogado) - SEM reconstruir o valor bruto, ao contrário dos
#scripts da pasta modelagem-artigo/. Sabemos que essa divisão introduz instabilidade
#estatística quando playing_time é pequeno (mesmo já filtrado > 1 min) - os
#pressupostos desses modelos tendem a ficar piores que os da versão bruta.
...
#modelo 1
mixed_model1 <- lmer(
  tot_dist ~ position + interval_c + quarter + age_c + playing_time_c +
...
#modelo 2
mixed_model2 <- lmer(
  tot_dist ~ position + interval_c + quarter + age_c + playing_time_c + playng_venue +
    (1 + quarter_lin | player) + (1 | match/qt_match) ,
  data = df, REML = FALSE
)
...
#modelo 3
mixed_model3 <- lmer(
  tot_dist ~ position + interval_c + quarter + age_c + playing_time_c + playng_venue + opponent_level_c +
    (1 + quarter_lin | player) + (1 | match/qt_match) ,
  data = df, REML = FALSE
)
...
#modelo 4
mixed_model4 <- lmer(
  tot_dist ~ position + interval_c + quarter + age_c + playing_time_c + playng_venue + opponent_level_c + score_dif_c +
    (1 + quarter_lin | player) + (1 | match/qt_match) ,
  data = df, REML = FALSE
)
...
tab_model(
  ...
  dv.labels = c("modelo 1 Total Distance/min", "modelo 2 Total Distance/min", "modelo 3 Total Distance/min", "modelo 4 Total Distance/min"),
  string.pred = "Preditores",
  string.est = "Estimativa (Beta)",
  title = "Tabela. Modelos Mistos para Total Distance por Minuto e Fatores Contextuais"
)
...
grafico_efeitos_fixos <- ggplot(efeitos_fixos, aes(x = estimate, y = reorder(term, estimate), color = sinal)) +
  ...
  labs(x = "Estimativa (Beta)", y = NULL, color = NULL,
       title = "Efeitos fixos - Modelo 4 (Total Distance/min)") +
...
grafico_trajetorias <- ggplot() +
  ...
  labs(x = "Quarto", y = "Distância total/min prevista",
       title = "Trajetória individual de distância total/min por quarto (modelo 4)",
...
grafico_caterpillar <- ggplot(ranef_tidy, aes(x = estimate, y = reorder(level, estimate), color = sinal)) +
  ...
  labs(x = "Desvio em relação à média", y = "Atleta", color = NULL,
       title = "Efeitos aleatórios por atleta - Modelo 4 (Total Distance/min)") +
