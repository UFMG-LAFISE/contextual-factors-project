...
#modelo 1
mixed_model1 <- lmer(
  dc_total_min ~ posicao + quarto_jg + idade + tempo + mando + classificacao + dif_duo + interv_dias +
    (1 | npf) + (1 | nome_sessao),
  data = df, REML = TRUE
)
...
#modelo 2 
mixed_model2 <- lmer(
  dcel2_min ~ posicao + quarto_jg + idade + tempo + mando + classificacao + dif_duo + interv_dias +
    (1 | npf) + (1 | nome_sessao),
  data = df, REML = TRUE
)
...
#modelo 3
mixed_model3 <- lmer(
  dcmi_min ~ posicao + quarto_jg + idade + tempo + mando + classificacao + dif_duo + interv_dias +
    (1 | npf) + (1 | nome_sessao),
  data = df, REML = TRUE
)
...
#modelo 4
mixed_model4 <- lmer(
  dci_min ~ posicao + quarto_jg + idade + tempo + mando + classificacao + dif_duo + interv_dias +
    (1 | npf) + (1 | nome_sessao),
  data = df, REML = TRUE
)
...
tab_model(
  ...
  dv.labels = c("Dcel. Total", "Dcel. Moderada", "Dcel. Moderada a Intensa", "Dcel. Intensa"),
  string.pred = "Preditores",
  string.est = "Estimativa (Beta)",
  title = "Tabela. Modelos Mistos para Número de Desacelerações e Fatores Contextuais"
)
