...
#modelo 1
mixed_model1 <- lmer(
  aci_min ~ posicao + interv_dias + quarto_jg + idade + tempo +
    (1 | npf) + (1 | nome_sessao),
  data = df, REML = FALSE
)
...
#modelo 2 
mixed_model2 <- lmer(
  aci_min ~ posicao + interv_dias + quarto_jg + idade + tempo + mando +
    (1 | npf) + (1 | nome_sessao),
  data = df, REML = FALSE
)
...
#modelo 3
mixed_model3 <- lmer(
  aci_min ~ posicao + interv_dias + quarto_jg + idade + tempo + mando + classificacao +
    (1 | npf) + (1 | nome_sessao),
  data = df, REML = FALSE
)
...
#modelo 4
mixed_model4 <- lmer(
  aci_min ~ posicao + interv_dias + quarto_jg + idade + tempo + mando + classificacao + dif +
    (1 | npf) + (1 | nome_sessao),
  data = df, REML = FALSE
)
...
#modelo 5
mixed_model5 <- lmer(
  aci_min ~ posicao + interv_duo + quarto_jg + idade + tempo + mando + classificacao + dif_duo + interv_dias +
    (1 | npf) + (1 | nome_sessao),
  data = df, REML = FALSE
)
...
tab_model(
  ...
  dv.labels = c("modelo 1 ACI_min", "modelo 2 ACI_min", "modelo 3 ACI_min", "modelo 4 ACI_min", "modelo 5 ACI_min"),
  string.pred = "Preditores",
  string.est = "Estimativa (Beta)",
  title = "Tabela. Modelos Mistos para Acelerações Intensas (>2 m/s2) por Minuto Jogado e Fatores Contextuais"
)
